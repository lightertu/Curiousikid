...
function SideBar({ playlist }: { playlist: Playlist }) {
  ...
  return (
    <li className="group relative">
      <Link
        prefetch={true}
        href={`/p/${playlist.id}`}
        className={`block py-1 px-4 cursor-pointer hover:bg-[#1A1A1A] text-[#d1d5db] focus:outline-none focus:ring-[0.5px] focus:ring-gray-400 ${pathname === `/p/${playlist.id}` ? 'bg-[#1A1A1A]' : ''
          }`}
        tabIndex={0}
      >
        {playlist.name}
    ...
    >
      <div className="m-4">
        ...
        <div className="mb-6">
          <Link
            href="/"
            className={`block py-1 px-4 -mx-4 text-xs text-[#d1d5db] hover:bg-[#1A1A1A] transition-colors focus:outline-none focus:ring-[0.5px] focus:ring-gray-400 ${pathname === '/' ? 'bg-[#1A1A1A]' : ''
              }`}
          >
            All Podcasts
          </Link>
      ...
      <ScrollArea className="h-[calc(100dvh-180px)]">
        ...
        >
          {playlists.map((playlist) => (
            <SideBar key={playlist.id} playlist={playlist} />
          ))}
        </ul>
      </ScrollArea>
    </div>
