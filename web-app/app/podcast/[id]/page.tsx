...
export default async function PodcastDetailPage({
  ...
  let podcast = await getPlaylistWithSongs(id);
    ...
    <div className="flex-1 flex flex-col overflow-hidden bg-[#0A0A0A] pb-[69px]">
      <div className="flex items-center justify-between p-3 bg-[#0A0A0A]">
        <div className="flex items-center space-x-1">
          ...
          <span className="text-sm">{podcast.name}</span>
        </div>
      ...
      <div className="flex items-center py-3 px-4 space-x-3 bg-[#0A0A0A]">
        <CoverImage url={podcast.coverUrl} playlistId={podcast.id} />
        <div>
          <EditableTitle playlistId={podcast.id} initialName={podcast.name} />
          <p className="text-xs sm:text-sm text-gray-400">
            {podcast.trackCount} tracks • {formatDuration(podcast.duration)}
          </p>
        </div>
      </div>
