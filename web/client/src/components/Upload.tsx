...
const Upload = ({ setUploadOpen }) => {
    ...
    const uploadFile = (file, index) => {
        ...
        uploadTask.on(
            ...
            (snapshot) => {
                ...
                // Make sure episodes[index].file is an object before assigning to uploadProgress
                const updatedEpisodes = [...podcast.episodes];
                if (typeof updatedEpisodes[index].file !== 'string') {
                    updatedEpisodes[index].file = {
                        ...file,
    ...
    useEffect(() => {
        if (podcast.episodes.length > 0 && podcast.episodes.every(episode =>
            episode.file !== "" &&
            episode.name !== "" &&
            episode.desc !== "" &&
            podcast.name !== "" &&
            ...
            podcast.thumbnail !== "")) {
            // Check if all episodes have URL files (string files) instead of File objects
                    ...
                    {!showEpisode ? (
                        ...
                        <>
                                        ...
                                        {podcast.episodes[index].file === "" ? (
                                            ...
                                            <Uploading>
                                                {typeof podcast.episodes[index].file === 'string' ? (
                                                    ...
                                                    <>
                                                        File: {(podcast.episodes[index].file as any).name}
                                                        <LinearProgress
                                                            sx={{ borderRadius: "10px", height: 3, width: "100%" }}
                                                            variant="determinate"
                                                            value={(podcast.episodes[index].file as any).uploadProgress || 0}
                                                            color={"success"}
                                                        />
                                                        {(podcast.episodes[index].file as any).uploadProgress || 0}% Uploaded
                                                    </>
                                                )}
                                            </Uploading>
                                        )}
                            ...
                            <OutlinedBox
                                ...
                                onClick={() =>
                                    setPodcast({ ...podcast, episodes: [...podcast.episodes, { name: "", desc: "", type: "audio", file: "" }] })
                                }
                            >
                                Add Episode
                            </OutlinedBox>
