...
class StoryService:
    ...
    def get_question_points(self, story_id: str) -> List[QuestionPoint]:
        return self.story_id_to_question_points.get(story_id)

    def get_question_point_context(self, question_point: QuestionPoint) -> str:
    ...
    def _load_stories_from_yaml(self, file_path: Path = DEFAULT_STORIES_FILE) -> List[StoryMetadata]:
    ...
    def _load_question_points_from_yaml(self, file_path: Path = DEFAULT_QUESTION_POINTS_FILE) -> List[QuestionPoint]:
        ...
        question_points = load_models_from_yaml(file_path, QuestionPoint)
        if not question_points:
            ...
            for question_point in question_points:
                question_points = id_to_question_points.get(question_point.storyId, []) # Get the list of question points for the story     
                question_points.append(question_point) # Add the question point to the list
                id_to_question_points[question_point.storyId] = question_points # Update the list of question points for the story
    ...
    def _load_transcriptions(self) -> Dict[str, StoryTranscription]:
        ...
        return {
            story.id: load_transcription_from_json(_get_story_transcription_file(story.id)) for story in self.id_to_story.values()
        }
