...
def generate_sound_design(
    ...
    if not claude_api_key and not openai_api_key:
        ...
        return create_default_sound_design(story_title, theme, key_elements, mood)
    ...
    if claude_api_key:
        try:
            ...
            if response.status_code == 200:
                ...
                # Parse the response
                sound_design = parse_sound_design(sound_design_text, story_title, theme)
                ...
                print(
                    f"Claude API rate limited (status 529). Attempting to use OpenAI fallback."
                )
                if openai_api_key:
                    ...
                    print(
                        "No OpenAI API key provided for fallback. Using default sound design."
                    )
...
def generate_sound_design_with_openai(
    ...
    try:
        ...
        if not prompt:
            ...
            story_desc = (
                story_description or f"{story_title} is a {theme} children's story"
            )
...
def parse_sound_design(sound_design_text, story_title, theme):
        ...
        for line in effect_lines:
            ...
            if line.startswith("-"):
                ...
                if position_match:
                    ...
                    effect_desc = re.sub(r"\(\d+%.*?\)", "", effect_line).strip()
    ...
    if not effects:
        ...
        for match in effect_matches:
            ...
            position = int(match.group(2)) / 100.0  # Convert percentage to decimal
        ...
        for i in range(positions_needed):
            ...
            if position not in sound_design["effect_positions"]:
                ...
                if position == 0.25:
                    ...
                    effect = (
                        f"Medium impact sound effect emphasizing {theme} story element"
                    )
                else:
                    effect = f"Distinctive sound effect for {story_title} story climax"

                sound_design["effects"].append(effect)
                sound_design["effect_positions"].append(position)
                print(f"Added generic effect at position {int(position * 100)}%")
...
def generate_audio_story(
    ...
    if not use_openai_voice and not elevenlabs_api_key:
        raise ValueError(
            "ElevenLabs API key must be provided when not using OpenAI for voice"
        )
    ...
    if story_transcript:
        ...
        if not claude_api_key and not openai_api_key:
            raise ValueError(
                "Either Claude or OpenAI API key must be provided even when using a transcript"
            )
    ...
    try:
        ...
        intro_stream = client.text_to_sound_effects.convert(text=sound_design["intro"])
        ...
        for i, effect_prompt in enumerate(sound_design["effects"]):
            effect_stream = client.text_to_sound_effects.convert(text=effect_prompt)
            effect_bytes = b"".join(chunk for chunk in effect_stream)

            effect_file = f"{components_folder}/{sanitized_title}_effect_{i + 1}.mp3"
        ...
        outro_stream = client.text_to_sound_effects.convert(text=sound_design["outro"])
        ...
        try:
            ...
            print(
                f"Warning: Couldn't export at 320k bitrate ({str(e)}), trying standard bitrate..."
            )
        ...
        with open(f"{components_folder}/README.txt", "w") as readme:
            ...
            readme.write(f"- {sanitized_title}_voice.mp3: The main voice narration\n\n")
            ...
            readme.write(f"- Fade-in duration: {volume_settings['effect_fade_in']}ms\n")
            readme.write(
            ...
            readme.write(
                f"- Crossfade duration: {volume_settings['crossfade']}ms for smooth transitions\n"
            )

        print(
            f"✓ {story_title} story generated successfully and saved to {output_path}"
        )
        print(f"✓ All audio components saved in the '{components_folder}' folder")
...
def get_openai_voice_instructions(mood, theme):
    ...
    emotion = emotion_instructions.get(
        theme.lower(),
        "Emotion: Warm and expressive, conveying the emotional journey of the story while maintaining a child-friendly delivery",
    )
...
def generate_simple_scene_descriptions(
    ...
    while len(template_descriptions) < num_frames:
        template_descriptions.append(
            f"{character} in {setting}, {mood} scene, children's book style"
        )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if openai_api_key:
                    ...
                    print(
                        "No OpenAI API key provided for fallback. Using simple story approach."
                    )
...
def get_volume_settings(theme, mood):
    ...
    if theme.lower() in ["adventure", "action", "explorer"]:
        ...
        settings[
            "effect_gain_overlay"
        ] = -5  # Slightly stronger futuristic effect presence
    elif theme.lower() in ["friendship", "family", "love", "togetherness"]:
        ...
        settings["effect_fade_in"] = (
            500  # Very long, gentler fades for relaxing stories
        )
    ...
    if mood.lower() in ["humorous", "funny", "light", "playful"]:
        ...
        settings["effect_fade_in"] = min(
            250, settings["effect_fade_in"]
        )  # Quicker fades for humorous mood
        settings["effect_fade_out"] = min(350, settings["effect_fade_out"])
        settings["effect_gain_overlay"] = max(
            -5, settings["effect_gain_overlay"]
        )  # Stronger effect presence for humor
    elif mood.lower() in ["emotional", "heartfelt", "touching"]:
        settings["background"] = min(
            21, settings["background"] + 2
        )  # Quieter background
        settings["crossfade"] = max(900, settings["crossfade"])  # Longer crossfades
        settings["effect_fade_in"] = max(
            450, settings["effect_fade_in"]
        )  # Longer fades for emotional content
        settings["effect_fade_out"] = max(550, settings["effect_fade_out"])
        settings["effect_gain_overlay"] = min(
            -9, settings["effect_gain_overlay"]
        )  # Very gentle effect introduction
    elif mood.lower() in ["excited", "energetic", "upbeat"]:
        settings["intro"] = max(1, settings["intro"] - 1)  # Louder intro
        settings["background"] = max(
            14, settings["background"] - 2
        )  # Louder background
        settings["effects"] = max(3, settings["effects"] - 2)  # Louder effects
        settings["crossfade"] = min(600, settings["crossfade"])  # Shorter crossfades
        ...
        settings["crossfade"] = max(900, settings["crossfade"])  # Longer crossfades
        settings["effect_gain_overlay"] = min(
            -8, settings["effect_gain_overlay"]
        )  # Gentler effect introduction

    # Make sure all settings are child-friendly (avoid too loud or startling settings)
    settings["effect_gain_overlay"] = max(
        settings["effect_gain_overlay"], -10
    )  # Not too loud
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Audio Story Generator for Children")
    ...
    parser.add_argument(
        "--elevenlabs-api-key",
        help="ElevenLabs API key (required if not using OpenAI voice)",
    )
    parser.add_argument(
        "--claude-api-key",
        help="Claude API key (used for story and sound design generation, will fall back to OpenAI if rate limited)",
    )
    parser.add_argument(
        "--openai-api-key",
        help="OpenAI API key (required if using OpenAI voice, also used as fallback for Claude when rate limited)",
    )
    parser.add_argument(
        "--pixellab-api-key", help="Pixellab API key (required for image generation)"
    )
    parser.add_argument(
        "--use-openai-voice",
        action="store_true",
    ...
    if not story_transcript and not args.claude_api_key and not args.openai_api_key:
        print(
            "Error: Either --claude-api-key or --openai-api-key must be provided for story generation"
        )
        exit(1)
    ...
    if args.generate_images and not args.pixellab_api_key:
        print(
            "Error: --pixellab-api-key must be provided when --generate-images is set"
        )
        exit(1)
