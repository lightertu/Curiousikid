...
class WhisperTranscriber:
    """Transcribes audio files listed in a stories YAML using OpenAI Whisper."""
    ...
    def _convert_audio_to_wav(self, story: StoryMetadata):
        ...
        if not source_audio_path.exists():
            ...
            raise FileNotFoundError(
                f"ERROR: Source audio file not found for story {story.id}: {source_audio_path}"
            )

        # Log the conversion process
        print(
            f"Converting {source_audio_path.name} to {wav_path.name} for story {story.id}"
        )
        ...
        try:
            (
                ffmpeg.input(str(source_audio_path))  # Input file
                .output(
                    str(wav_path), acodec="pcm_s16le", ac=1, ar="16000"
                )  # Output WAV, force mono, 16kHz
                .run(overwrite_output=True, quiet=True)  # Execute conversion
            )
            print(
                f"Converted {source_audio_path.name} to {wav_path.name} for story {story.id}"
            )
            return wav_path
        except ffmpeg.Error as e:
            print(
                f"ERROR converting {source_audio_path.name} for story {story.id}: {e}"
            )
    ...
    def _convert_all_audio_to_wav(self):
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            list(
                tqdm(
                    executor.map(self._convert_audio_to_wav, self.stories),
    ...
    def _transcribe_wav(self, story: StoryMetadata):
        ...
        if not wav_path.exists():
            print(
                f"Skipping transcription for story {story.id}: WAV file not found at {wav_path}"
            )
            return

        if processed_json_path.exists():
            print(
                f"Processed transcription already exists for story {story.id}: {processed_json_path}"
            )
            return
        ...
        try:
            ...
            print(
                f"Processed transcription saved for story {story.id}: {processed_json_path}"
            )
        except IOError as e:
    ...
    def _transcribe_all_wav(self):
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            list(
                tqdm(
                    executor.map(self._transcribe_wav, self.stories),
                    total=len(self.stories),
                )
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Transcribe stories defined in a metadata YAML using Whisper."
    )

    parser.add_argument(
        "--id",
        type=str,
        help="ID of the story to transcribe",
    )
    ...
    try:
        transcriber = WhisperTranscriber(model_name="large-v3", id=args.id)
