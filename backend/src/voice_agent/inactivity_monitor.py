...
class InactivityMonitor:
    ...
    async def monitor_interaction(self):
        while True:
            ...
            is_inactive_timeout = time.time() - self.state.last_interaction_time >= PROMPT_WARNING_TIME

            if not self.state.is_active and is_inactive_timeout:
                logger.info("Sending follow up prompt")
                self.state.silence_follow_up_retry = self.state.silence_follow_up_retry - 1
