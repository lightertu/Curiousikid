...
from memory.story.models import StoryMetadata, CurrentStory, ProactiveQuestionPoint
...
class DeviceState(BaseModel):
    ...
    stories: List[StoryMetadata] = Field(
        default_factory=lambda: INIT_STORY_LIST
    )  # Use loaded list
    ...
    currentStory: Optional[CurrentStory] = Field(
        default_factory=lambda: CurrentStory(
            **INIT_STORY_LIST[0].model_dump(), currentTime=0.0
        )
        if INIT_STORY_LIST
        else None
    )
    ProactiveQuestionPoint: Optional[ProactiveQuestionPoint] = None
...
if __name__ == "__main__":
    ...
    print(
        "Initial current story:",
        store.currentStory.title if store.currentStory else "None",
    )
    ...
    if new_store.stories:
        ...
        updated_store = new_store.update_subtree(
            path_to_first_story_title, "A New Title for Birdy"
        )
